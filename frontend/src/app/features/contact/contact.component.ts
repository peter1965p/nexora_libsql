  ...
  template: `
    <div class="pt-20">
      ...
      <section class="py-24 pb-32">
        <div
          class="container-main grid grid-cols-1 lg:grid-cols-[1fr_1.6fr] gap-24"
        >
          <!-- Info -->
          <div>
            <h2 class="text-2xl font-bold font-display mb-8">
              Direkter Kontakt
            </h2>
            <div class="space-y-4">
              @for (info of infos; track info.title) {
                <div class="card-base p-5 flex gap-4 items-start">
                  ...
                  <div>
                    <strong
                      class="block text-[0.7rem] text-white/50 uppercase tracking-[0.08em] mb-1 font-semibold"
                    >
                      {{ info.title }}
                    </strong>
                    <p
                      class="text-white/75 text-sm leading-relaxed m-0"
          ...
          <div class="bg-card border border-subtle rounded-2xl p-12">
            @if (successMessage()) {
              <div class="text-center py-16">
                <div
                  class="w-16 h-16 bg-[#28c840]/10 border border-[#28c840]/30 rounded-full
                            flex items-center justify-center text-[#28c840] text-2xl mx-auto mb-6"
                >
                  ✓
                </div>
              ...
              <form [formGroup]="form" (ngSubmit)="onSubmit()" novalidate>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
                  <div>
                    <label
                      class="block text-[0.7rem] font-semibold text-white/60 uppercase tracking-[0.06em] mb-2"
                      for="name"
                      >Name *</label
                    >
                    <input
                    ...
                    />
                    @if (isInvalid("name")) {
                      <span class="text-[#ff5f57] text-xs mt-1 block"
                        >Bitte geben Sie Ihren Namen ein.</span
                      >
                    }
                  </div>
                  <div>
                    ...
                    @if (isInvalid("email")) {
                      <span class="text-[#ff5f57] text-xs mt-1 block"
                        >Bitte eine gültige E-Mail angeben.</span
                      >
                    }
                  </div>
                </div>
                ...
                <div class="mb-6">
                  ...
                  <textarea
                    ...
                    [class.border-[#ff5f57]]="isInvalid('message')"
                  ></textarea>
                  @if (isInvalid("message")) {
                    <span class="text-[#ff5f57] text-xs mt-1 block"
                      >Mindestens 10 Zeichen erforderlich.</span
                    >
                  }
                </div>
                ...
                @if (errorMessage()) {
                  <div
                    class="bg-[#ff5f57]/10 border border-[#ff5f57]/30 rounded-[10px]
  ...
  infos = [
    {
      icon: "📍",
      title: "Adresse",
      value: "Päffgen IT<br>Lindenhof<br>54531 Manderscheid",
    },
    {
      icon: "✉️",
      title: "E-Mail",
      value:
        '<a href="mailto:peter@paeffgen-it.de" class="text-accent no-underline">peter@paeffgen-it.de</a>',
    },
    {
      icon: "📞",
      title: "Telefon",
      value:
        '<a href="tel:+4915569448813" class="text-accent no-underline">+49 155 69 44 88 13</a>',
    },
        ...
        next: (res) => {
          ...
          if (res.success)
            this.successMessage.set(
              res.data?.message ?? "Vielen Dank für Ihre Nachricht!",
            );
          else
            this.errorMessage.set(res.error ?? "Ein Fehler ist aufgetreten.");
        },
        error: () => {
          this.isLoading.set(false);
          this.errorMessage.set(
